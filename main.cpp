...
//void loadimage(void){
//    for(int j=1;j<2;j++){///2->4
//        //QString dir="C://Users//Administrator//Desktop//calibrate_pic//";
//        //QString num="%1//"
//        for(int i=0;i<42;i++){///1->0

//            ///cv::Mat image=cv::imread(QString("C://Users//jo//Desktop//calibrate_pic//%1//cam_%2.png").arg(j).arg(i, 2, 10, QLatin1Char('0')).toStdString());
//            cv::Mat image=cv::imread(QString("C://Users//jo//Desktop//decode//original software make the patterns_10bit//pat_%1.png").arg(i, 2, 10, QLatin1Char('0')).toStdString());
...
//    bool all_found = true;
//    for (unsigned i=0; i<count; i++)//只 取每个子目录下的第一张图像
//    {
...
int main(int argc, char *argv[])
{
    ...
    //loadimage();
    ///calibrate();
    //测试用imshow("hello",APP->_sum_pattern_to_corner.at(0).at(0)/*  _sum_pattern_to_corner.at(0).at(0)*/);

 //cv::Mat image=cv::imread(QString("C://Users//jo//Desktop//calibrate_pic//%1//cam_%2.png").arg(2).arg(1, 2, 10, QLatin1Char('0')).toStdString());

//    pattern_list.resize(1);
//    min_max_list.resize(1);
//    cv::Mat & pattern_image = pattern_list[0];
//    cv::Mat & min_max_image = min_max_list[0];
//    if (!decode_gray_set(0, pattern_image, min_max_image))//解码,并输出解码图案：pattern_image
//    {   //error
//        std::cout << "ERROR: Decode image set " << 0 << " failed. " << std::endl;
//        //return;
//    }
